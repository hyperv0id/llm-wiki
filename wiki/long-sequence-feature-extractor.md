...
## 两个模块
- **上下文感知线性化 Transformer**：先用膨胀时间卷积提取点级局部上下文（弥补单点语义弱），再用 [[performer|Performer]] 的**正随机特征 ([[positive-random-features|PRF]])** 近似 softmax 核 exp(qᵀk)≈φ(q)ᵀφ(k)——PRF 是 Performer 论文提出的 softmax 核正特征无偏估计器、正交化可进一步降方差[^src-performer]；因 Σφ(k)v、Σφ(k) 跨查询共享，复杂度从 O(T_l²) 降到 **O(T_l)**；以**生成式预训练**（由长历史预测未来）学习长程时间动态[^src-bigst]。
