...
TiDE sits between the [[ltsf-linear|LTSF-Linear / DLinear]] challenge to Transformers and the [[patchtst|PatchTST]] revival of attention via patching+CI. Relative to DLinear it adds non-linear residual capacity and first-class covariates; relative to PatchTST it trades attention for linear-time dense maps and stronger covariate highways. Later exogenous-aware models (e.g., TimeXer, ExoTST, KITE, DAG) often still cite TiDE as a strong MLP/covariate baseline for long-horizon forecasting.[^src-tide]
...
- Related models: [[ltsf-linear]], [[patchtst]], [[informer]], [[autoformer]], [[fedformer]]
