...
Wang & Du 等人的 MTSI 综述将 CSDI 归为生成式-扩散类插补方法（Table 1 架构标注 Diffusion+Attention+CNN，缺失机制标注 MCAR），并称其为首个专门为 MTSI 设计的扩散模型；综述同时概括了 CSDI 的条件训练方式——以观测数据的子集作为条件信息生成其余部分——并指出其去噪网络依赖两个 Transformer、对变量数与序列长度呈二次复杂度，带来内存压力[^src-mts-imputation-survey]。该概括与本页"长序列 $O(L^2)$ 成本"局限一致，也是 SSSD 以结构化状态空间模型替换 Transformer 的直接动机[^src-mts-imputation-survey]。
