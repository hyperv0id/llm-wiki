...
离散 DLM（如 MDLM、Duo）直接在离散 token 空间中定义扩散过程（absorbing state、uniform transition 等）。连续 DLM 则借助连续空间来绕开离散扩散的数学复杂性[^src-elf-embedded-language-flows]。
...
这些方法的共同特点：**训练时每步做离散化**（cross-entropy loss），本质上没有真正利用连续空间的灵活性[^src-elf-embedded-language-flows]。
...
这些方法开始探索不同的连续状态空间，但仍然在训练或推理时做每步离散化，或者依赖单独 decoder[^src-elf-embedded-language-flows]。
...
这些方法与 [[ELF]] 是并发工作，但它们仍然在轨迹中每步使用 token 级 cross-entropy 监督[^src-elf-embedded-language-flows]。
...
- 直接使用冻结的 contextual embedding[^src-elf-embedded-language-flows]

## 设计空间

ELF 论文提供了对连续 DLM 设计空间的系统 survey（附录 Tab.2），涵盖以下设计轴[^src-elf-embedded-language-flows]：
