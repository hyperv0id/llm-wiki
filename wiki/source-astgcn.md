...
1. **Spatial-Temporal Attention Module**:
   ...
   - **Temporal Attention** — learns a dynamic attention matrix E ∈ ℝ<sup>T<sub>r−1</sub>×T<sub>r−1</sub></sup> that captures varying correlations between different time slices.[^src-source-astgcn]
...
2. **Spatial-Temporal Convolution Module**:
   ...
   - **Temporal Convolution** — standard 1D convolution along the time dimension after graph convolution.[^src-source-astgcn]
...
- ASTGCN's advantage grows with prediction horizon length, confirming that dynamic attention is especially valuable for long-term forecasting[^src-source-astgcn]
