...
FaST 是一个针对大规模时空图（STG）长视野预测的高效且有效的框架，发表在 KDD 2026。论文提出两个关键创新：[^src-source-fast-long-horizon-forecasting]
...
- GLU expert 相比 FFN expert 提升 1.4x 推理速度且保持精度[^src-source-fast-long-horizon-forecasting]
...
- 内存复杂度与节点数线性相关可扩展到 8600 节点[^src-source-fast-long-horizon-forecasting]

## 与现有页面的关系

- 继承自 [[source-hyperd-hybrid-periodicity-decoupling|HyperD]] 的周期性建模思路但扩展到大规划图
- 与 [[traffic-forecasting]] 领域的 STGCN、DCRNN 等传统方法对比
- 作为 [[mixture-of-experts]] 在时空图预测中的具体应用[^src-source-fast-long-horizon-forecasting]
...
[^src-source-fast-long-horizon-forecasting]: [[source-fast-long-horizon-forecasting]]