...
但“白噪声方差均匀”不等于“各频率 SNR 均匀”。若 $y_0=F x_0$ 且 $C_i=\operatorname{Var}[(y_0)_i]$，DDPM 的频率 SNR 为 $s_t^{\mathrm{DDPM}}(i)=\bar\alpha_t C_i/(1-\bar\alpha_t)$；当 $C_i$ 呈傅里叶功率律衰减时，高频 SNR 会比低频更快下降。[^src-equal-snr]
...
[[equal-snr|EqualSNR]] 是一种以 SNR 为目标的频域噪声控制：它把一般前向过程写成 $y_t=\sqrt{\bar\alpha_t}y_0+\sqrt{1-\bar\alpha_t}\epsilon_\Sigma$，并令 $\Sigma_{ii}=cC_i$，从而使 $s_t(i)=\bar\alpha_t C_i/((1-\bar\alpha_t)\Sigma_{ii})$ 与频率 $i$ 无关。[^src-equal-snr] 它与两频段混合噪声的区别是：EqualSNR 不是手工强调某个频段，而是补偿数据天然频谱方差，使所有频率按相同 SNR 速率被破坏。[^src-equal-snr]
...
SAGD（Spectrally Anisotropic Gaussian Diffusion）是同一团队提出的完整版框架，将频域噪声控制形式化为各向异性高斯协方差 $\Sigma_w$。核心理论贡献包括：推导各向异性下的通用 score-$\epsilon$ 关系、证明 $\Sigma_w \succ 0$ 时 $t \to 0$ score 收敛到真实数据 score、以及 rank-deficient $\Sigma_w$ 下 projected score 的选择性忽略理论。详见 [[source-sagd]]。[^src-sagd]
