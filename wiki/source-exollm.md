...
ExoLLM 与 [[source-exost]]（时空预测中外生变量的 select-then-balance 范式）、[[source-timexer]] / [[source-crosslinear]]（数值型 many-to-one 外生预测）、[[source-nbeatsx]]（早期可解释外生神经基扩展）和 [[source-e2-cstp]]（因果多模态时空预测）同属外生变量建模方向，但各有侧重：
- ExoST 关注数值型外生变量的选择与平衡，未涉及 LLM 或文本模态。
- TimeXer / CrossLinear 在数值外生序列上做 endo–exo 融合（注意力 vs 1D conv residual），不使用语言知识。
- NBEATSx 用残差基扩展 + TCN/直接外生基分解数值协变量（EPF），无可解释堆栈之外的 LLM/文本通道。
