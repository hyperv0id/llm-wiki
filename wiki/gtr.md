...
- **[[ts-memory|TS-Memory]]**（KDD 2026）同为即插即用的检索增强预测方法，但 TS-Memory 将 kNN 检索蒸馏为参数化记忆模块，推理时无需检索；GTR 的检索为可学习参数矩阵按绝对位置检索，推理时仍含检索操作但 $O(1)$[^src-ts-memory-time-series-foundation-models-kdd26]。
