...
TCP-Diffusion（Tropical Cyclone Precipitation Diffusion）是首个基于深度学习的全球热带气旋（TC）降水预测模型，由 Huang, Mu, Bai & Watson（浙江工业大学 + 布里斯托大学）发表于 ICML 2025[^src-tcp]。模型以 TC 中心周围 10°×10° 区域为目标，基于过去 12h 观测和多模态环境变量，以 3h 间隔预测未来 12h 的降水场。是 [[tropical-cyclone-precipitation-forecasting|TC 降水预测]]领域的开创性工作。
...
- 1877 个全球 TC（1980-2020，六大洋区），126 个测试 TC（2018-2020）
- ETS-24（中等降水 >24 mm/3h）：0.147 vs PreDiff 0.119，提升 ~23%
- ETS-60（强降水 >60 mm/3h）：0.00644，唯一超越 Persistence 的 DL 模型
- 超越 ECMWF-IFS：ETS-6 0.412 vs 0.302，TPM AE 0.474 vs 0.507
- 消融实验：ARP（+0.1~15.0%）、多模态数据（+5.6~13.0%）、NWP 引导（+2.0~9.3%）

## 局限

预测时效仅 12h；扩散模型推理较慢（1.253 s/sample）；依赖 ERA5-IFS 外生输入；TC 形成期和消散期预测性能较弱[^src-tcp]。
