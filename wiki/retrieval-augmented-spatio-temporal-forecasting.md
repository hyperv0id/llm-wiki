...
- **PIR**（Post-forecasting Identification and Revision，Liu et al., NeurIPS 2025）是纯时序的轻量实例检索修订：后处理阶段在训练输入–目标对上以实例归一化编码 + 余弦相似度检索 top-K 相似实例、softmax 加权求和作为全局修订项，不经生成模型；与 RAST 的双维 FAISS 检索、RATD 的扩散参照引导均不同[^src-pir]。
