...
创建的页面：[[source-time-indexed-imputation]], [[tabpfn-ts]], [[motm]], [[time-indexed-foundation-model]]
更新的页面：[[nuwats]], [[csdi]], [[index]], [[log]]

## [2026-06-08] ingest | PRDIM — Missing Pattern Recognized Diffusion Imputation Model for Missing Not At Random (arXiv 2026)

Ingest PRDIM paper (Gyuwon Sim, Sumin Lee, Heesun Bae, Byeonghu Na, Doyun Kwon, Il-Chul Moon [KAIST]; Ju-Hee Hwang, Jae-Young Lim [SNU]; arXiv:2605.25439v1, May 2026, preprint). PRDIM is a diffusion imputation framework专攻 MNAR (Missing Not At Random) — missingness depends on the unobserved values themselves, making the missing process non-ignorable (must model p_φ(M|X^obs,X^mis)). Most diffusion imputers (CSDI et al.) instead assume MCAR artificial masking and train/eval p(X|X^obs,A,A); PRDIM shows imputing original (real) missing is far harder than artificial, and closes the gap. Method: jointly model p_θ(X)p_φ(M|X), maximize joint likelihood of (X^obs,M) via EM (X^mis latent). Pattern recognizer D_φ (discriminator à la GAIN/not-MIWAE, BCE) approximates p(M|X); ELBO under diffusion (Prop 3.1) folds it into the diffusion objective; hard EM (vs DiffPuter's soft EM) for exploration; EM monotonicity guaranteed (Cor 3.2). Two phases: Phase 1 diffusion pre-training with adjacent target masking (artificial missing placed near original missing, not MCAR); Phase 2 EM iteration — M-step trains θ+φ, E-step generates X^mis with pattern-recognizer guidance (Prop 3.3: score = diffusion score − ∇L_PR, a classifier-guidance analogue using Tweedie posterior mean; random recognizer gives neutral guidance early). Results across 3 modalities (TS: ETT/STOCK/PEMS-Bay; image: FMNIST/CelebA-HQ; 5 UCI tabular) vs 10 baselines: improves over cDiffPuter on original missing (RMSE 1.209→1.057, MAE 0.782→0.663), gains strongest out-of-sample; recovers semantic image structure; beats DiffPuter on tabular. Ablations: removing pattern recognizer or hard EM both degrade significantly; under MCAR the advantage vanishes (MNAR-specific benefit). Created MNAR concept page as a hub for the imputation cluster (records each model's missing-mechanism assumption). Code/overhead: gradient guidance adds moderate inference cost, recognizer lightweight.

创建的页面：[[source-prdim]], [[prdim]], [[missing-not-at-random]], [[pattern-recognizer-guidance]]
更新的页面：[[csdi]], [[classifier-guidance]], [[nuwats]], [[t1]], [[index]], [[log]]

## [2026-06-08] ingest | T1 — One-to-One Channel-Head Binding for Multivariate Time-Series Imputation (ICLR 2026)

Ingest T1 paper (Dongik Park, Hyunwoo Ryu, Suahn Bae, Keondo Park, Hyung-Sin Kim; Seoul National University; ICLR 2026, arXiv:2602.21043v4). T1 ("Time series imputation with 1-to-1 channel-head binding") is a CNN-Transformer hybrid for multivariate imputation under heavy/diverse missingness. Core insight: robust imputation needs both (1) temporal extraction from sparse per-variable observations and (2) selective cross-variable transfer without importing noise — existing methods excel at one, compromise the other (time-axis / variable-axis / dual-axis tokenization / temporal CNN each have a structural flaw). Key mechanism — Channel-Head Binding (CHead Attention): 1-to-1 correspondence between CNN channels and attention heads (n_h=C); shared depthwise convs make channel c extract the same temporal pattern across all variables (semantically aligned), and each head does cross-variable attention on only its bound channel — so when missingness corrupts a channel, its head down-weights it without contaminating other channels. Architecture: Mask-Aware Embedding (observed-only instance norm + series⊕mask 2-channel conv + variable encoding) → B × T1 Block (Temporal Conv QKV with shared multi-scale DWConv → CHead Attention → Conv FFN) → PixelShuffle1D reconstruction upsampler. Self-supervised (40% observed masked), seq len 96, consistent hyperparameters across all datasets. Results on 11 datasets vs 11 baselines: SOTA, −46% avg MSE vs 2nd-best PatchTST (point missing), −48% (block missing), −23%/−13% on natural PhysioNet2012/AQI36; at 0.7 missing T1 MSE ≈ half of PatchTST; trained at 0.4, generalizes to unseen 0.1–0.7 without retraining; only 0.543M params. Ablations: remove cross-variable −56.16%, attention→PWConv −12.91% (adaptive > fixed transfer), non-1-to-1 binding (8/16/32 ch/head) +7.45/16.86/14.57%. Design-philosophy contrast with NuwaTS: T1 = task-aligned bespoke architecture (inductive bias), NuwaTS = PLM reuse + scale. Code: github.com/Oppenheimerdinger/T1.

创建的页面：[[source-t1]], [[t1]], [[channel-head-binding]]
更新的页面：[[nuwats]], [[imputeformer]], [[csdi]], [[index]], [[log]]

## [2026-06-08] ingest | NuwaTS — a Foundation Model Mending Every Incomplete Time Series (arXiv 2024)

Ingest NuwaTS paper (Jinguo Cheng, Chunwei Yang, Wanlin Cai, Yuxuan Liang, Yuankai Wu, Qingsong Wen; Sichuan University / HKUST-GZ / Squirrel Ai; arXiv:2405.15317v3, Oct 2024). NuwaTS repurposes the first 6 layers of a PLM (GPT-2 default) into a one-for-all incomplete time series imputation foundation model — the first claimed to generalize across domains. Core contributions: (1) variable-wise train/val/test benchmark (partition along variable dimension 1:1:1, not time) to assess cross-variable + cross-domain generalization, contrasting conventional time-wise splits; (2) three specialized embeddings — statistical (min/median/max/trend at series + patch level), missing (learnable vector × per-patch mask ratio), domain-specific prefix — plus mask-invariant contrastive learning (two mask-ratio views, InfoNCE bi-linear + MSE); (3) plug-and-play prefix fine-tuning (P-tuning-v2 style, [Key,Value]=P+βK̂, β=0.01, injected every layer, <100KB vs 331.77MB, removable). Four versions: specific / one-for-all (17.6M fused samples) / fine-tuned / cross-domain (LargeST 100.1M). Results: one-for-all beats domain-specific SOTA (SAITS/BRITS/TimesNet/PatchTST/GPT4TS) at nearly all 9 missing rates over 10 datasets; scaling law for imputation; CI enables zero-shot to different variable counts (LargeST⇒ECL/Weather); 10% fine-tune data = 100% on ETT; robust to continuous missing; impute→forecast boosts TimesNet; converts to forecaster via masked padding tokens. Key ablation findings: from-scratch (no NLP weights) sharply degrades zero-shot transfer (cross-modality pretraining is meaningful); data quantity > inductive biases; GPT-2 > BERT > LLaMA2; simple linear embedding > Time-LLM-style text-alignment for incomplete series. Limitation: fixed length-96, struggles with longer/entirely-missing segments. Code: github.com/Chengyui/NuwaTS.

创建的页面：[[source-nuwats]], [[nuwats]], [[variable-wise-partitioning]], [[plug-and-play-prefix-tuning]]
更新的页面：[[contrastive-learning]], [[channel-independence]], [[time-llm]], [[patch-reprogramming]], [[model-reprogramming]], [[imputeformer]], [[instance-normalization]], [[csdi]], [[index]], [[log]]

## [2026-06-08] ingest | TimeDiT — General-Purpose Diffusion Transformers for Time Series Foundation Model (KDD 2025)
