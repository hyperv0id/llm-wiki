...
1. **识别（Identify）**：不确定性估计器 δ = f_ue(x, ȳ, E) 以逐实例 MSE 为代理预测误差，误差越大的实例获得越大的修订权重（机制见 [[error-based-uncertainty-estimation]]）[^src-pir]。
...
- **[[test-time-computing-st]]（ST-TTC，NeurIPS 2025）**：同样在冻结骨干后追加后处理模块，但 TTC 利用时序标签自相关在推理期在线校准，PIR 随骨干离线训练、推理期不更新——机制不同，共享"不动骨干"的插件精神。
