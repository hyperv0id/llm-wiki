...
Published at the DeepSpatial '22 workshop (ACM SIGKDD) by Chengxin Wang, Yuxuan Liang, and Gary Tan (National University of Singapore), PRNet addresses **crowd flow forecasting** — predicting the number of people entering or leaving city regions. The core observation is that while raw crowd flow data is highly dynamic and non-stationary, the **periodic residual** (the difference between the current time step and the same time step one period ago, e.g., one week) is far more stationary and easier to learn. PRNet reframes crowd flow forecasting as a periodic residual learning problem.[^src-prnet]
